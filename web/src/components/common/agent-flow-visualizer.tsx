...
import { ShieldCheck, Brain, Zap, Scale, Users, SearchCode, History, Lightbulb, Gavel, FileInput, FileOutput } from "lucide-react";
  ...
  return (
    <div className={cn("flex items-center justify-center", className)}>
      <div className={lineClasses}>
        {isAnimating && (
          <div
            className={cn("data-packet-sm bg-accent shadow-[0_0_8px_theme(colors.accent)]", packetClasses)} 
            style={style}
          />
        )}
      </div>
...
const AgentFlowVisualizer: React.FC<AgentFlowVisualizerProps> = ({ isAnalyzing }) => {
  return (
    <div className="p-4 md:p-6 rounded-xl border border-border/50 bg-background/30 w-full flex flex-col items-center space-y-3 md:space-y-4 overflow-x-auto">
      ...
      <FlowArrow direction="down" isAnimating={isAnalyzing} lengthClass="h-8 md:h-12" flowDistance="2rem" />
      <AgentNode name="Cerebrum (Orchestrator)" icon={Brain} isAnimating={isAnalyzing} className="border-primary shadow-primary/30" />
      <FlowArrow direction="down" isAnimating={isAnalyzing} lengthClass="h-6 md:h-8" flowDistance="1.5rem" />
      ...
      <FlowArrow direction="down" isAnimating={isAnalyzing} lengthClass="h-4 md:h-6" flowDistance="1rem" />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-3 md:gap-x-4 gap-y-4 md:gap-y-6 w-full max-w-5xl px-1">
        ...
        {agents.map((agent, index) => (
          <div key={agent.name} className="flex flex-col items-center space-y-1 md:space-y-2">
            <FlowArrow direction="down" isAnimating={isAnalyzing} lengthClass="h-4 md:h-6" delay={`${index * 0.1}s`} flowDuration="0.8s" flowDistance="1rem" />
            <AgentNode name={agent.name} icon={agent.icon} isAnimating={isAnalyzing} iconClassName={agent.color.replace('text-', '')} />
            <FlowArrow direction="up" isAnimating={isAnalyzing} lengthClass="h-4 md:h-6" delay={`${0.5 + index * 0.1}s`} flowDuration="0.8s" flowDistance="-1rem" />
          </div>
        ))}
      </div>

      <FlowArrow direction="up" isAnimating={isAnalyzing} lengthClass="h-4 md:h-6" flowDistance="-1rem" />
      {/* Collector Line */}
      <div className="w-full max-w-4xl h-px bg-slate-600 relative">
         {isAnalyzing && (
          <>
      ...
      <FlowArrow direction="up" isAnimating={isAnalyzing} lengthClass="h-6 md:h-8" flowDistance="-1.5rem" />
